...
#else
                    var spectrum2 = new float[numSamples];
                    AudioListener.GetSpectrumData(spectrum, sampleChannel, windowUsed); //get the spectrum data
                    AudioListener.GetSpectrumData(spectrum2, 1 - sampleChannel, windowUsed); //get the spectrum data

                    for (int i = 0; i < spectrum.Length; i++)
                    {
                        spectrum[i] = (spectrum[i] + spectrum2[i]) / 2;//这里才让他听两个声道
                    }
