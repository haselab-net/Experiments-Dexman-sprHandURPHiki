...
public class PHSolidBehaviour : SprSceneObjBehaviour {
    ...
    // UnityのOnValidate
    public float controlledWeight = 1; //为了实验后加的，为了不让人知道这个是质量
    public override void OnValidate() {
        desc.mass = controlledWeight;
        if (desc.mass == 0) {
            desc.mass = 1e-5;
    ...
    public void UpdatePosition(Vector3 newPosition, Quaterniond ori){ //后写的,用于重生，移动位置
        if (sprObject != null) {
    ...
    public void UpdatePositionOnly(Vector3 newPosition){ //后写的,用于重生，移动位置
        if (sprObject != null) {
            PHSolidIf so = sprObject as PHSolidIf;
            so.SetVelocity(new Vec3d(newPosition.x, 0, newPosition.z));
            //gameObject.transform.position = newPosition;
            so.SetPose(gameObject.transform.ToPosed());
        }
    }

    public void suddenDown(){ //后写的,用于重生，移动位置
        if (sprObject != null) {
            PHSolidIf so = sprObject as PHSolidIf;
            so.SetVelocity(new Vec3d(0, -3f, 0));
        }
    }
